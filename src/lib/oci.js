...
export async function getBitmapInscriptionAndType(address) {
    ...
    if (parcelNumber !== null) {
        ...
        // For regular bitmaps, use the bitmap's sat index
        atIndex = getBitmapSatIndex(bitmapNumber);
    }
    
    const idResp0 = await fetch(origin + '/r/sat/' + sat + '/at/0').then(r => r.json());
    const idRespLast = await fetch(origin + '/r/sat/' + sat + '/at/-1').then(r => r.json());
    const inscriptionId0 = idResp0.id;
    const inscriptionIdLast = idRespLast.id;
    const isBitmap = (inscriptionId0 === inscriptionIdLast);
    
    return { inscriptionId: inscriptionIdLast, isBitmap };
}